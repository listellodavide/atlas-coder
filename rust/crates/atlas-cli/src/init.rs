...
const GITIGNORE_COMMENT: &str = "# Atlas Code local artifacts";
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to Atlas Code (atlascode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
