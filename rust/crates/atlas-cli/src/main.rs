   ...
   Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mAlt+Enter / Ctrl+O\x1b[0m for newline",
    ...
    fn handle_repl_command(
        ...
        Ok(match command {
            ...
            SlashCommand::Resume {
                session_path,
                model,
            } => self.resume_session(session_path, model)?,
            SlashCommand::Restore { session_path } => self.resume_session(session_path, None)?,
    ...
    fn run_rlm(&mut self, task: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for i in 1..=30 {
            ...
            if assistant_text.contains("TASK_COMPLETED") {
                println!("✅ RLM successfully completed the task.");
                break;
            }

            if i == 30 {
                println!("⚠️ RLM reached the 30 iteration limit without finishing.");
                break;
            }

            current_prompt = "Please verify your work. Next, continue executing your plan. If you are fully done, answer with 'TASK_COMPLETED'.".to_string();
        }
        Ok(())
    }
    ...
    fn run_squad(&mut self, task: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if !model.starts_with("ollama/") && !model.contains("ollama") {
            println!(
                "\x1b[33m⚠️  /squad uses Ollama agents. Switch to a local model first:\x1b[0m"
            );
            println!("  /ollama qwen2.5-coder:14b");
            println!("Then retry: /squad {task}");
            return Ok(());
        }
    ...
    fn resume_session(
        ...
        let model = if let Some(m) = model_override {
            ...
            resolved
        } else if let Some(session_model) = &session.model {
            let m = session_model.clone();
    ...
    fn run_mematlas(&mut self, action: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match action {
            Some(a) if a.starts_with("learn") => {
                let path = a.trim_start_matches("learn").trim();
...
fn format_plan_report(mode: Option<&str>) -> String {
    format!(
        "Plan
  Mode             {}
  Action           Toggle or inspect planning mode for task sequencing
  Output           Status report of planning constraints and next steps",
        mode.unwrap_or("inspect")
    )
}
