...
fn slash_command_names_match_known_commands_and_suggest_nearby_unknown_ones() {
    ...
    // when
    let help_output = Command::new(env!("CARGO_BIN_EXE_atlas"))
        .current_dir(&temp_dir)
        .arg("/help")
        .output()
        .expect("atlas should launch");
    let unknown_output = Command::new(env!("CARGO_BIN_EXE_atlas"))
        .current_dir(&temp_dir)
        .arg("/zstats")
        .output()
        .expect("atlas should launch");
...
fn config_command_loads_defaults_from_standard_config_locations() {
    ...
    fs::create_dir_all(temp_dir.join(".atlas")).expect("project config dir should exist");
    ...
    fs::write(temp_dir.join(".atlas.json"), r#"{"model":"sonnet"}"#)
        .expect("write project settings");
    fs::write(
        temp_dir.join(".atlas").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    let output = command_in(&temp_dir)
        .env("ATLAS_CONFIG_HOME", &config_home)
        .args([
            "--resume",
            session_path.to_str().expect("utf8 path"),
            "/config",
    ...
    assert!(stdout.contains(temp_dir.join(".atlas.json").to_str().expect("utf8 path")));
    assert!(stdout.contains(
        temp_dir
            .join(".atlas")
