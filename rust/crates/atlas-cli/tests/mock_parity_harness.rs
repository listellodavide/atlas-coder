...
fn run_case(case: ScenarioCase, workspace: &HarnessWorkspace, base_url: &str) -> ScenarioRun {
    ...
    command
        ...
        .env("ATLAS_CONFIG_HOME", &workspace.config_home)
    ...
    let output = if let Some(stdin) = case.stdin {
        ...
        child.wait_with_output().expect("atlas should finish")
    } else {
...
fn prepare_auto_compact_fixture(workspace: &HarnessWorkspace) {
    let sessions_dir = workspace.root.join(".atlas").join("sessions");
...
fn prepare_plugin_fixture(workspace: &HarnessWorkspace) {
    ...
    fs::write(
        &script_path,
        "#!/bin/sh\nINPUT=$(cat)\nprintf '{\"plugin\":\"%s\",\"tool\":\"%s\",\"input\":%s}\\n' \"$ATLASD_PLUGIN_ID\" \"$ATLASD_TOOL_NAME\" \"$INPUT\"\n",
    )
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "atlas-mock-parity-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
