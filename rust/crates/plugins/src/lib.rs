...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        process
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .env("ATLASD_PLUGIN_ID", &self.plugin_id)
            .env("ATLASD_PLUGIN_NAME", &self.plugin_name)
            .env("ATLASD_TOOL_NAME", &self.definition.name)
            .env("ATLASD_TOOL_INPUT", &input_json);
        if let Some(root) = &self.root {
            process
                .current_dir(root)
                .env("ATLASD_PLUGIN_ROOT", root.display().to_string());
        }
