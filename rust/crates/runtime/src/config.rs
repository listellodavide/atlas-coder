...
pub const ATLAS_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".atlas.json"),
            |parent| parent.join(".atlas.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".atlas.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".atlas").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".atlas").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("ATLAS_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".atlas")))
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".atlas.json");
...
mod tests {
    use super::{
        ...
        RuntimePluginConfig, ATLAS_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".atlas");
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".atlas");
        fs::create_dir_all(cwd.join(".atlas")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".atlas.json"),
        ...
        assert_eq!(ATLAS_SETTINGS_SCHEMA_NAME, "SettingsSchema");
