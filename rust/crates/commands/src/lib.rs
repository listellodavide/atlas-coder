...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        ...
        argument_hint: Some("<session-path> [model] | with model <model> [path]"),
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "config",
...
SlashCommandSpec {
    ...
    SlashCommandSpec {
        name: "restore",
        aliases: &[],
        summary: "Restore a session along with its previously used model",
    ...
    SlashCommandSpec {
        name: "rlm",
        aliases: &[],
        summary: "Execute task in 30 autonomous plan -> execute -> verify loops",
        argument_hint: Some("<task>"),
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "squad",
        aliases: &[],
        summary: "Run a 5-agent dev pipeline (Plan→Review→Code→Test) via Ollama",
        argument_hint: Some("<task>"),
        resume_supported: false,
    },
    SlashCommandSpec {
...
pub fn validate_slash_command_input(
    ...
    Ok(Some(match command {
        ...
        "resume" => {
            if args.is_empty() {
            ...
            } else if args.len() >= 3 && args[0] == "with" && args[1] == "model" {
                // Handle /resume with model <model> [path]
                let model = Some(args[2].to_string());
                let session_path = if args.len() > 3 {
                ...
                let session_path = Some(args[0].to_string());
        ...
        "restore" => SlashCommand::Restore {
            session_path: optional_single_arg(command, &args, "[session-id]")?,
        },
        ...
        "rlm" => {
            if args.is_empty() {
                return Err(command_error(
                    "Missing task description for rlm.",
                    command,
                    "/rlm <task>",
                ));
            }
        ...
        "squad" => {
            if args.is_empty() {
                return Err(command_error(
                    "Missing task description for squad.",
                    command,
                    "/squad <task>",
                ));
            }
...
mod tests {
    ...
    fn parses_supported_slash_commands() {
        ...
        assert_eq!(
            SlashCommand::parse("/resume with model gemma:latest"),
            Ok(Some(SlashCommand::Resume {
                session_path: Some("latest".to_string()),
                model: Some("gemma:latest".to_string()),
            }))
        );
        assert_eq!(
            SlashCommand::parse("/resume with model gemma:latest old-session.jsonl"),
            Ok(Some(SlashCommand::Resume {
                session_path: Some("old-session.jsonl".to_string()),
                model: Some("gemma:latest".to_string()),
            }))
        );
        assert_eq!(
        ...
        assert_eq!(
            SlashCommand::parse("/resume session.json gemma:latest"),
            Ok(Some(SlashCommand::Resume {
                session_path: Some("session.json".to_string()),
                model: Some("gemma:latest".to_string()),
            }))
    ...
    #[test]
    fn ollama_parses_with_and_without_model() {
        // /ollama without a model name yields None
        assert_eq!(
            SlashCommand::parse("/ollama"),
            Ok(Some(SlashCommand::Ollama { model: None }))
        );

        // /ollama with a single model name
        assert_eq!(
            SlashCommand::parse("/ollama llama3"),
            Ok(Some(SlashCommand::Ollama {
                model: Some("llama3".to_string()),
            }))
        );

        // /ollama with a multi-word model tag (spaces joined)
        assert_eq!(
    ...
    #[test]
    fn rlm_parses_task_description() {
        // /rlm with a single-word task
        assert_eq!(
            SlashCommand::parse("/rlm refactor"),
            Ok(Some(SlashCommand::Rlm {
                task: "refactor".to_string(),
            }))
        );

        // /rlm with a multi-word task description
        assert_eq!(
            SlashCommand::parse("/rlm add unit tests for the parser"),
            Ok(Some(SlashCommand::Rlm {
                task: "add unit tests for the parser".to_string(),
            }))
        );
    }

    #[test]
    fn rlm_requires_task_description() {
        let error = SlashCommand::parse("/rlm")
            .expect_err("rlm without a task should be rejected")
            .to_string();
        assert!(
            error.contains("Missing task description for rlm"),
